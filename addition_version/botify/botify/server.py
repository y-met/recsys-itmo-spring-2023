...
from botify.recommenders.user_track_embd import UserTrackEMBD
...
embeddings = {
    "context": load_pickle(os.path.join(app.config["EMBEDDINGS_FOLDER"], "context.pcl")),
    "track": load_pickle(os.path.join(app.config["EMBEDDINGS_FOLDER"], "track.pcl")),
    "user": load_pickle(os.path.join(app.config["EMBEDDINGS_FOLDER"], "user.pcl"))
}
...
class NextTrack(Resource):
    def post(self, user: int):
        ...
        treatment = Experiments.USERTRACKEMBD.assign(user)
        if treatment == Treatment.T1:
            recommender = UserTrackEMBD(tracks_redis.connection, catalog, embeddings)
